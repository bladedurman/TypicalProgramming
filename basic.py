...
class Parser:
    ...
    def atom(self):
        ...
        if tok.type in (TT_INT, TT_FLOAT):
            ...
            if res.error:
                # .failure(InvalidSyntaxError(tok.pos_start, tok.pos_end, "Expected '('"))
                return res
            if self.current_tok.type == TT_RPAREN:
                return res.success(expr)
            else:
                return res.failure(InvalidSyntaxError(tok.pos_start, tok.pos_end, "Expected ')'"))
        return res.failure(InvalidSyntaxError(self.tokens[self.tok_idx - 1].pos_start, self.tokens[self.tok_idx - 1].pos_end, "Expected int, float, identifier, '+' '-' or '('"))
    ...
    def expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(self.tokens[self.tok_idx - 1].pos_end, self.tokens[self.tok_idx - 1].pos_end, "Expected 'var', int, float, identifier, '+', '-', or '('"))
